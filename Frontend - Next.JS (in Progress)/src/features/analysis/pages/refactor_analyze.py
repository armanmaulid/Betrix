...
file_path = r"D:\Betrix\Frontend - Client\src\features\analysis\pages\AnalyzePage.tsx"

with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# Replace Imports
import_block = """import { useState, useRef, useEffect, useCallback } from "react";
...
# Find the start of export function AnalyzePage()
idx_start = content.find('export function AnalyzePage() {')
if idx_start != -1:
    content = import_block + "\n\n" + content[idx_start:]

# Inside AnalyzePage()
new_body = """export function AnalyzePage() {
  ...
  const { view, setView, messages, setMessages, setCurrentSessionId, isStreaming } = useChatStore();
              ...
              <div className="flex flex-col gap-6 max-w-4xl mx-auto pb-4">
                {messages.map((msg: any, idx: number) => (
                  <ChatMessageItem
                    ...
                    tools={msg.tools}
                    cost={msg.cost}
                    isFinishedGlow={msg.isFinishedGlow}
                  />
                ))}
...
idx_end = content.find('export function AnalyzePage() {')
if idx_end != -1:
    content = content[:idx_end] + new_body
