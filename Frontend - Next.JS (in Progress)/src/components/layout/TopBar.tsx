  ...
  return (
    <div className="flex items-stretch border-b border-[var(--border)] bg-[var(--surface)] text-[11px]">
      ...
      <div className="flex flex-shrink-0 items-center gap-1.5 border-l border-[var(--border)] px-3 text-[10px]">
        <span className="text-[#b5a679]">API: <span className={`font-bold ${(process.env.NODE_ENV !== 'production') ? "text-yellow-500" : "text-[var(--success)]"}`}>{(process.env.NODE_ENV !== 'production') ? "DEVELOPMENT" : "OPERATIONAL"}</span> <span className="text-[var(--text-muted)]">v0.1</span></span>
      </div>
