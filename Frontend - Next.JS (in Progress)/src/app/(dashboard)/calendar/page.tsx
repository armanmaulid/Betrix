...
import { fetchEconomicCalendar, type CalendarEvent } from "@/features/market/api/marketClient";
import { getSharedEventSource } from "@/features/market/hooks/useTickerPrices";
...
function StatCard({ icon, label, value, tone, onClick }: { icon: React.ReactNode; label: string; value: number; tone: string; onClick?: () => void }) {
  return (
    <div 
      onClick={onClick}
      className={`flex items-center gap-2 border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 ${onClick ? 'cursor-pointer hover:bg-[var(--surface-alt)]' : ''}`}
    >
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--surface)] py-2 pr-4">
          <StatCard 
            ...
            onClick={() => setFilter(prev => ({ ...prev, impacts: new Set(["high"]) }))}
          />
          ...
          <StatCard 
            ...
            onClick={() => setFilter(prev => ({ ...prev, impacts: new Set(["high", "medium", "low"]) }))}
          />
        </div>
            ...
            Object.entries(groups).map(([dateKey, groupEvents]) => {
              ...
              return (
                <div key={dateKey} data-date-key={dateKey}>
                  ...
                  {past.map((ev) => (
                    <EventRow key={`p-${ev.eventId}`} ev={ev} />
                  ))}
                  {showNowLine && (
                  ...
                  {future.map((ev) => (
                    <EventRow key={`f-${ev.eventId}`} ev={ev} />
                  ))}
                </div>
              );
            })
...
function EventRow({ ev }: { ev: CalendarEvent }) {
  ...
  return (
    <div
      className={
        CAL_COLS + " border-b border-[var(--border)] py-1.5 text-[11px] transition-colors duration-1000 " +
        (flash ? "bg-[var(--accent)]/30" : "hover:bg-[var(--surface-alt)]")
      }
    >
