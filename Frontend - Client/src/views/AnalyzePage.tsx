...
import { streamChat, getChatHistory, deleteChatSession } from "../lib/api/chatClient";
import { fetchCandles, type BrokerSymbol, fetchBrokerSymbols } from "../lib/api/marketClient";
import { getNews, type NewsItem } from "../lib/api/newsClient";
import {
  INSTRUMENT_COMMANDS,
...
export function AnalyzePage() {
  const { setRightPanel, setOnSearch } = useShellStore();
  ...
  const handleSearchSymbol = useCallback((s: string) => {
    router.push(`/?symbol=${s.toUpperCase()}`);
  }, [router]);
      ...
      <div className="flex flex-col flex-1 bg-[#050505] overflow-hidden font-mono text-[13px] text-[#ccc]">
        ...
        {view === 'landing' ? (
          ...
          <div className="flex flex-col flex-1 overflow-hidden relative animate-slide-up-fade">
            ...
            {/* MESSAGES */}
            <div className="flex flex-col flex-1 overflow-y-auto py-6 gap-6">
