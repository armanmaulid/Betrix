...
export class PgVerificationRepository implements VerificationRepository {
  async create(userId: string, token: string, ttlSeconds: number, newEmail?: string): Promise<void> {
    await pgClient.query(
      `INSERT INTO email_verifications (user_id, token, expires_at, new_email) VALUES ($1, $2, NOW() + INTERVAL '1 second' * $3, $4)`,
      [userId, token, ttlSeconds, newEmail || null]
    );
  }
